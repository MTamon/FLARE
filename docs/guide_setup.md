...
**使用フォーク**: [MTamon/DECA@cuda128](https://github.com/MTamon/DECA/tree/cuda128) (CUDA 12.8 対応版)

#### セットアップ (推奨: セットアップスクリプト)

```bash
# DECA 専用スクリプト (サブモジュール初期化 + CUDA 12.8 依存インストール + チェックポイント取得)
bash install/setup_deca.sh

# または FLAME ルート全体を一括セットアップ
...
```bash
# サブモジュールの初期化 (初回のみ)
git submodule update --init third_party/DECA
# → third_party/DECA/ に MTamon/DECA@cuda128 がチェックアウトされる

# CUDA 12.8 対応パッケージのインストール
cd third_party/DECA
bash install_128.sh
cd ../..
...
extractor:
  ...
  repo_dir: ./third_party/DECA  # サブモジュールパス (sys.path に追加)
```
...
**使用フォーク**: [MTamon/FlashAvatar@release/cuda128-fixed](https://github.com/MTamon/FlashAvatar/tree/release/cuda128-fixed) (CUDA 12.8 対応版)

#### セットアップ (推奨: セットアップスクリプト)

```bash
# FlashAvatar 専用スクリプト (サブモジュール初期化 + pytorch3d/3DGS ビルド)
...
```bash
# サブモジュールの初期化 (再帰: diff-gaussian-rasterization, simple-knn 含む)
git submodule update --init --recursive third_party/FlashAvatar
# → third_party/FlashAvatar/ に MTamon/FlashAvatar@release/cuda128-fixed がチェックアウト

# CUDA 12.8 対応パッケージのインストール (pytorch3d + diff-gaussian-rasterization のビルドを含む)
...
学習手順の詳細は [DECA + FlashAvatar 統合ガイド](guide_deca_flashavatar.md#4-flashavatar-モデルの学習) を参照してください。
...
renderer:
  ...
  repo_dir: ./third_party/FlashAvatar  # サブモジュールパス (sys.path に追加)
```
