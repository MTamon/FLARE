#!/usr/bin/env python
"""Webカメラリアルタイム DECA + FlashAvatar デモ。

SMIRK 版 ``demo_webcam.py`` の DECA 置換版。Webカメラまたは動画ファイル
(ideal-source モード) から毎フレーム:
    1. MediaPipe で顔検出
    2. DECA で FLAME パラメータ抽出 (shape/exp/pose/cam のみ)
    3. MediaPipe Face Landmarker で eyes_pose / eyelids を補完推定
    4. DECAToFlameAdapter(use_mediapipe_supplement=True) でパラメータ変換
    5. FlashAvatar で 3DGS レンダリング

DECA は眼球ポーズ・瞼パラメータを出力しないため、SMIRK 版と異なり MediaPipe
Face Landmarker による補完が必須となる (handover §5 参照)。補完が失敗した
場合は identity 回転 × 2 と zero eyelids にフォールバックする。
...
各ステージのタイマをオーバーレイ表示 (cap / mp / deca / eye / cvt / render / disp)。
終了時に集計サマリを表示。
...
WSL2 環境:
    MediaPipe GPU delegate が使用不可なため起動時に警告し、CPU にフォールバックする。
...
Usage:
    python demos/demo_webcam_deca.py \\
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Webカメラリアルタイム DECA + FlashAvatar デモ",
    ...
    p.add_argument(
        "--no_render",
        action="store_true",
        help="FlashAvatar レンダリングをスキップ (DECA のみベンチマーク)",
    )
    p.add_argument(
        "--no_eye_supplement",
        action="store_true",
        help="MediaPipe による eyes_pose / eyelids 補完を無効化 (DECA 本来挙動を維持)",
    )
    p.add_argument(
        "--snapshot_dir",
        default="output",
...
def _draw_overlay(
    ...
    lines = [
        ...
        f"cap:{t('cap')}  mp:{t('mp')}  deca:{t('deca')}",
        f"eye:{t('eye')}  cvt:{t('cvt')}  render:{t('render')}",
        f"disp:{t('disp')}",
        f"mp:{mp_delegate}  dev:{device_str}  eye_sup:{'ON' if eye_supplement else 'OFF'}",
...
def main() -> None:
    ...
    # モデルロード
    logger.info("DECA ロード中: {}", args.deca_model)
    if not Path(args.deca_model).exists():
        ...
        sys.exit(1)
    deca = _load_deca(args.deca_model, device)

    from flare.converters.deca_to_flame import DECAToFlameAdapter
    eye_supplement = not args.no_eye_supplement
    adapter = DECAToFlameAdapter(use_mediapipe_supplement=eye_supplement)
    ...
    logger.info("eye supplement: {}", "ON (MediaPipe Face Landmarker)" if eye_supplement else "OFF (DECA 本来挙動)")
    ...
    agg: dict[str, float] = {k: 0.0 for k in ("cap", "mp", "deca", "eye", "cvt", "render", "disp")}
    agg_face = 0
    ...
    try:
        while True:
            ...
            if face_ok and bbox is not None:
                ...
                try:
                    ...
                    with torch.no_grad():
                        params = deca.extract(tensor)
                    timings["deca"] = (time.perf_counter() - t0) * 1000.0
                    ...
                    timings["eye"] = (time.perf_counter() - t0) * 1000.0
            ...
            _draw_overlay(canvas, fps, timings, face_ok, args.mp_delegate, device, eye_supplement)
            ...
            if key == ord("s"):
                ...
                snap = os.path.join(args.snapshot_dir, f"snapshot_deca_{ts}.png")
        ...
        stages = ("cap", "mp", "deca", "eye", "cvt", "render", "disp")
        ...
        logger.info(
            "mp_delegate={}  device={}  eye_supplement={}",
            args.mp_delegate, device, eye_supplement,
        )
